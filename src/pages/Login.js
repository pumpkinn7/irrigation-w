...
function Login() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (!userDoc.exists()) {
        await signOut(auth); // ล็อกเอาท์ทันทีถ้าไม่พบข้อมูลผู้ใช้
        setError('บัญชีนี้ยังไม่ได้ลงทะเบียน กรุณาสมัครสมาชิก');
        return;
      }
      ...
      if (auth.currentUser) {
        await signOut(auth); // ล็อกเอาท์ถ้าเกิดข้อผิดพลาด
      }
            ...
            <div className="card shadow">
              <div className="card-body p-4">
                ...
                <div className="alert alert-warning py-2 mb-3">
                  ...
                  กรุณาเข้าสู่ระบบด้วยบัญชี Google ที่ได้ลงทะเบียนไว้
                </div>
                ...
                <div className="text-center mt-2">
                  <p className="mb-0">ยังไม่มีบัญชี? <Link to="/register" className="text-info text-decoration-none">สมัครสมาชิก</Link></p>
                </div>
              </div>
            </div>
          </div>
